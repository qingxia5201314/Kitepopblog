...
import { createPostComment, listPostComments, listPosts } from './blogApi';
...
describe('blog api client', () => {
  ...
  it('reads and creates public post comments', async () => {
    ...
    await createPostComment('hello-post', { nickname: 'Kite', role: '读者', content: '不错' });
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(2, '/api/posts/hello-post/comments', {
      ...
      body: JSON.stringify({ nickname: 'Kite', role: '读者', content: '不错' })
    });
  });
});
